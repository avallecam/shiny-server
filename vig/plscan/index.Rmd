---
title: "CDC Perú: Scan Statistics - Malaria"
output: 
...
malariadb_summary <- malariadb_nominal %>% 
  mutate(time_strata=case_when(
    var_year==max(var_year) ~ "year_last",
    TRUE ~ "year_prev"
  )) %>% 
  # filter(time_strata=="year_last") %>% 
  # count(var_time02)
  group_by(ubigeo,time_strata,var_year) %>% 
...
scanstat_01 <- function(var_name_depa="LORETO") {
...
# n_depa <- 2
name_depa <- var_name_depa #"LORETO"
...
complete_time_range <- (real_range$min %m+% years(2)):(real_range$max) #complete time rage
cluster_time_range <- (real_range$max %m-% years(1)):(real_range$max) #time range of interes
